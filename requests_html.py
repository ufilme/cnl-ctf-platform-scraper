...
class HTML(BaseParser):
    ...
    async def _async_render(self, *, url: str, script: str = None, scrolldown, sleep: int, wait: float, reload, content: Optional[str], timeout: Union[float, int], keep_page: bool, localStorage: [dict] = None):
        ...
        try:
            ...
            if localStorage:
                ...
                for pair in localStorage:
                    local_storage_script += f"() => {{ localStorage.setItem('{list(pair.keys())[0]}', '{list(pair.values())[0]}'); }}\n"
                await page.evaluateOnNewDocument(local_storage_script)
    ...
    def render(self, retries: int = 8, script: str = None, wait: float = 0.2, scrolldown=False, sleep: int = 0, reload: bool = True, timeout: Union[float, int] = 8.0, keep_page: bool = False, localStorage: [dict] = None):
        ...
        for i in range(retries):
            if not content:
                try:
                    ...
                    content, result, page = self.session.loop.run_until_complete(self._async_render(url=self.url, script=script, sleep=sleep, wait=wait, content=self.html, reload=reload, scrolldown=scrolldown, timeout=timeout, keep_page=keep_page, localStorage=localStorage))
                except TypeError:
                    pass
            else:
                break
